...
(function () {
  ...
  /* ---------------------------------------------------------------------
     1-C. SALA — código da partida e sorteio compartilhado

     No modo online as duas equipes precisam receber a MESMA empresa. Em vez
     de o servidor sortear e distribuir, o nome é derivado do código da sala:
     mesmo código, mesmo nome, calculado nas duas pontas sem ida à rede.
     Uma coisa a menos para sincronizar — e o sorteio continua funcionando
     com a internet caindo no meio.

     O alfabeto do código não tem O/0 nem I/1: o código é dito em voz alta
     numa sala de aula, e esse é o par que sempre se confunde.
     --------------------------------------------------------------------- */

  var ROOM_ALPHABET = "ACDEFGHJKLMNPQRTUVWXY34679";
  ...
  function newRoomCode() {
    ...
    if (window.crypto && window.crypto.getRandomValues) {
      ...
      for (i = 0; i < ROOM_CODE_LENGTH; i++) {
        out += ROOM_ALPHABET.charAt(bytes[i] % ROOM_ALPHABET.length);
      }
      return out;
    }
    for (i = 0; i < ROOM_CODE_LENGTH; i++) out += pickRandom(ROOM_ALPHABET.split(""));
    return out;
  }

  /* Perdoa o que é seguro perdoar — minúscula, espaço, hífen — e recusa o
     resto. Nada de adivinhar que o "O" digitado "era pra ser Q": mandar a
     equipe para a sala errada em silêncio é pior do que pedir para digitar
     de novo, porque as duas só descobririam no fim, com empresas
     diferentes na tela. Devolve "" quando não dá para confiar. */
  function normalizeRoomCode(raw) {
    var code = String(raw || "").toUpperCase().replace(/[\s-]+/g, "");
    if (code.length !== ROOM_CODE_LENGTH) return "";
    for (var i = 0; i < code.length; i++) {
      if (ROOM_ALPHABET.indexOf(code.charAt(i)) < 0) return "";
    }
    return code;
  }

  /* FNV-1a: espalha bem para strings curtas e cabe em cinco linhas. */
  ...
  function companyForRoom(code) {
    return COMPANY_NAME_SUGGESTIONS[hashCode(code) % COMPANY_NAME_SUGGESTIONS.length];
  }
  ...
  /* A licença só sai a partir daqui. Entre 50 e 69 a análise trava e a
     exigência volta; abaixo de 50 o processo é arquivado — nos dois casos
     NÃO há deferimento. É por isso que vencer o duelo não basta: o duelo
     compara as duas equipes, o deferimento é padrão absoluto. */
  var LICENSE_MIN = 70;

  /* Cronômetro. Só o modo online liga (setTimer): no individual e no jogo
     de sala, sem adversário, relógio só atrapalha quem está aprendendo. */
  var QUESTION_SECONDS = 60;
  var SETUP_SECONDS = 120;
  ...
  /* Conta pelo relógio de parede, não somando intervalos: aba em segundo
     plano estrangula setInterval, e um cronômetro que atrasa junto com a
     aba daria vantagem a quem trocasse de janela. */
  function startCountdown(selector, secondsLeft, onEnd) {
    if (tick) { clearInterval(tick); tick = null; }
    var el = $(selector);
    if (!el) return;
    ...
    function paint() {
      var left = Math.max(0, Math.ceil(secondsLeft()));
      el.textContent = clockText(left);
      el.classList.toggle("cronometro--urgente", left <= 10);
      if (left <= 0) {
        clearInterval(tick);
        tick = null;
        onEnd();
  ...
  /* Prazo local: vale para a decisão, que é de cada equipe. */
  function startTimer(selector, seconds, onEnd) {
    var deadline = Date.now() + seconds * 1000;
    startCountdown(selector, function () {
  ...
  /* assets/img/ufra.png embutida: fundo branco removido, recortada e reduzida para
  ...
  /* assets/img/adm.png, mesmo tratamento */
  ...
  /* O pior que uma decisão pode render. Serve para a escala da nota final e
     para o estouro de prazo, que pesa igual a errar — se fossem duas contas
     separadas, um dia elas discordariam. */
  function worstPointsFor(decision) {
    if (decision.type === "choice") {
      return Math.min.apply(null, decision.options.map(function (o) { return o.points; }));
    }
    if (decision.type === "sorting") return decision.cards.length * decision.pointsPerMiss;
    return MULTI_SELECT_FAIL_POINTS;
  }

  var MIN_POINTS = CONTENT.reduce(function (t, f) {
    return t + f.decisions.reduce(function (s, decision) {
      return s + worstPointsFor(decision);
    }, 0);
  }, 0);
  ...
  function renderDecision() {
    ...
    $("#btn-registrar").addEventListener("click", function () { submitDecision(false); });

    if (timerEnabled) {
      startTimer("#cronometro", QUESTION_SECONDS, function () { submitDecision(true); });
    }
  }
  ...
  /* Prazo vencido pesa o mesmo que errar. Valer zero seria pior: deixar o
     relógio correr passaria a ser melhor do que arriscar, que é o oposto do
     que o jogo ensina. */
  function evaluateTimeout(decision) {
    return {
      verdict: "erro",
      points: worstPointsFor(decision),
      effects: { compliance: -6, credibility: -4 },
      text: "O prazo desta decisão venceu sem manifestação. No processo real o " +
        "silêncio não é neutro: a exigência continua de pé e o ônus segue com o " +
        "empreendedor. Por isso não decidir custa o mesmo que decidir errado.",
  ...
  function submitDecision(byTimeout) {
    stopTimer();
    var outcome = byTimeout === true ? evaluateTimeout(currentDecision()) : evaluate();
    var entries = [];
  ...
  /* Se o prazo do cadastro vence com vagas em branco, a partida não pode
     travar: entra um nome genérico e o processo abre. */
  function fillMissingTeamNames() {
    teamDraft.forEach(function (c, i) {
      if (c.name.trim()) return;
      c.name = "Responsável " + pad2(i + 1);
      var field = $("#equipe-nome-" + i);
      if (field) field.value = c.name;
    });
  }
  ...
  function startTeamGame() {
    ...
    /* No online quem dá a largada é a sala: esta equipe só se declara pronta
       e espera a outra. onTeamReady devolve true quando assume o comando. */
    var bridge = window.AssinaEmbaixo;
    if (bridge && typeof bridge.onTeamReady === "function" &&
        bridge.onTeamReady(company, teams) === true) {
      return;
    }
  ...
  var SCREENS = ["#tela-boas-vindas", "#tela-online", "#tela-equipes",
                 "#tela-abertura", "#tela-jogo", "#tela-largada",
                 "#tela-duelo", "#tela-final"];
  ...
  function showScreen(selector) {
    ...
    else if (selector === "#tela-largada") $("#faixa-folha").textContent = "Largada";
  ...
  function startGame(name, teams) {
    stopTimer(); /* partida nova nunca herda relógio da anterior */
  ...
  /* O parecer de uma nota, para o modo online descrever cada lado sem
     recalcular banda nenhuma — a régua tem que ser a mesma dos dois lados. */
  function gradeFor(score) {
    var band = GRADE_BANDS.filter(function (f) { return score >= f.min; })[0];
    return { title: band.title, text: band.text, granted: score >= LICENSE_MIN };
  }
  ...
  window.AssinaEmbaixo = {
    ...
    setTimer: function (on) { timerEnabled = !!on; if (!on) stopTimer(); },
    /* O prazo do cadastro é da SALA: online.js entrega uma função que
       calcula quanto falta pelo relógio do servidor, para as duas equipes
       contarem para o mesmo instante. Sem ela — nunca deveria acontecer —
       cai num prazo local em vez de ficar sem relógio nenhum. */
    startSetupTimer: function (secondsLeft) {
      var deadline = Date.now() + SETUP_SECONDS * 1000;
      startCountdown("#cronometro-cadastro", secondsLeft || function () {
        return (deadline - Date.now()) / 1000;
      }, function () {
        fillMissingTeamNames();
        startTeamGame();
      });
    },
    beginGame: function (company, teams) { startGame(company, teams); },
    onTeamReady: null,
